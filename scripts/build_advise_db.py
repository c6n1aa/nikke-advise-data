"""从咨询 JSON 生成 SQLite 数据库 dist/advise.db（全量覆盖，不做增量）。

源数据（人工维护的唯一真源）是 data/nikke_advises_i18n.json，
...
用法（纯标准库，默认路径锚定到仓库根，任意工作目录均可运行）：
    ...
    python scripts\\build_advise_db.py --source <json> --target <db>

构建产物 dist/advise.db 是派生文件（gitignore）。发布 = 复制覆盖 ok-nikke 的
assets/db/advise.db 并在 ok-nikke 提交。
"""
...
DEFAULT_SOURCE = os.path.join(_REPO_ROOT, 'data', 'nikke_advises_i18n.json')
DEFAULT_TARGET = os.path.join(_REPO_ROOT, 'dist', 'advise.db')
